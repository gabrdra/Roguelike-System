...
static func validate_map(map_data:MapData) -> bool:
	...
	for level_name:String in duplicated_map_data.levels:
		...
		if validated_level == false:
			var message:= "There was an error in "+level_name
			printerr(message)
			still_valid = false
...
#
static func _validate_level(level:LevelData, passages_holder_name:String) -> bool:
	var still_valid := true
	for room_name:String in level.rooms:
		var room:Room = level.rooms[room_name]
		if !_validate_room(room, passages_holder_name):
			still_valid = false
	return still_valid #_generate_level_possibilities(level)
#
...
static func _validate_room_passages(room:Room) -> bool:
