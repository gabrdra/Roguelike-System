...
static func _add_instance_to_graph(root_node:ViablePathGraphNode, end_node:ViablePathGraphNode, instance_array:Array[int]) -> void:
	...
	var back_index:int
	for i in range(instance_array.size()-1, 0, -1):
		back_index = i
		if last_node_from_back.parents.has(instance_array[back_index-1]):
			var parent_node:ViablePathGraphNode = last_node_from_back.parents[instance_array[back_index-1]]
			for child_index in range(parent_node.children.size()):
				if parent_node.children[child_index].connection_pair_id == last_node_from_back.connection_pair_id:
					parent_node.children_frequency[child_index]+=1
					break
