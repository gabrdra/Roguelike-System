...
func _get_unused_connections_from_room(room:Room) -> SimpleDeque:
	...
	while not passages_names.is_empty():
		...
		deque.insert_back(Connection.new(room, passage_name))
	return deque
...
func generate_level(input_level:LevelData, input_seed:int = 0) -> LevelData:
	...
	var deque = _get_unused_connections_from_room(used_rooms[input_level.starter_room.name])
	var successful := true
	while successful:
		if deque.is_empty():
			break
		var outgoing_connection:Connection = deque.pop_front()
		...
		if not used_rooms.has(incomming_connection.room.name):
			...
			# and it's passages added to the deque
			...
			deque.insert_deque_back(_get_unused_connections_from_room(new_used_room))
		else:
...
class ErrorCorrection:
	var parent_room_name:String #room that inserted this room into the final map
	var room:Room
	var passages_attempts:Dictionary
	var deque_copy:SimpleDeque
